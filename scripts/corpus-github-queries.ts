...
function buildGenericQueries(): QueryDef[] {
  ...
  const yamlFormats = [
    { ext: "yaml", openapi: OPENAPI3_YAML, openapi30: OPENAPI30_YAML, openapi31: OPENAPI31_YAML, crud: CRUD_POST },
    { ext: "yml", openapi: OPENAPI3_YAML, openapi30: OPENAPI30_YAML, openapi31: OPENAPI31_YAML, crud: CRUD_PUT },
  ] as const;
...
function buildFrameworkQueries(): QueryDef[] {
  ...
  return [
    { group: "frameworks", name: "fastapi", q: `filename:openapi.yaml fastapi ${OPENAPI3_YAML} ${CRUD_POST}` },
    { group: "frameworks", name: "springdoc", q: `filename:openapi.yaml springdoc ${OPENAPI3_YAML} ${CRUD_POST}` },
    { group: "frameworks", name: "laravel", q: `filename:openapi.yaml laravel ${OPENAPI3_YAML} ${CRUD_POST}` },
    { group: "frameworks", name: "ktor", q: `filename:openapi.yaml ktor ${OPENAPI3_YAML} ${CRUD_POST}` },
  ];
}
...
function buildVendorQueries(): QueryDef[] {
  const vendors = [
    "stripe", "plaid", "adyen", "twilio", "sendgrid", "mailgun", "slack", "notion",
    "linear", "asana", "github", "gitlab", "circleci", "vercel", "digitalocean",
    "cloudflare", "datadog", "openai", "anthropic", "postman", "rapidapi",
  ] as const;
