...
describe('extractArrayFromResponse', () => {
  ...
  it('extracts array from results path', () => {
    ...
    expect(extractArrayFromResponse(body, 'results')).toEqual([{ id: 1 }, { id: 2 }]);
  });

  it('extracts array from users path', () => {
    const body = { users: [{ id: 1, name: 'Alice' }] };
    expect(extractArrayFromResponse(body, 'users')).toEqual([{ id: 1, name: 'Alice' }]);
  });
  ...
  it('supports dot-notation for nested path', () => {
    ...
    expect(extractArrayFromResponse(body, 'response.items')).toEqual([{ id: 1 }]);
  });
});
