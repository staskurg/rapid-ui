...
import { computeMultiSpecDiff, computeSpecDiff, type UISpecMap } from '@/lib/spec/diff';
import { formatDiffForDisplay, formatMultiSpecDiffForDisplay } from '@/lib/spec/diffFormatters';
...
describe('computeSpecDiff', () => {
  ...
  it('detects changed field', () => {
    const next: UISpec = {
      ...baseSpec,
      fields: baseSpec.fields.map((f) => (f.name === 'name' ? { ...f, label: 'Full Name' } : f)),
    };
    const diff = computeSpecDiff(baseSpec, next);
