...
function checkOpenAPI(content: string): {
  valid: boolean;
  errors?: { code: string; message: string; jsonPointer?: string }[];
} {
...
describe('check-openapi (parse → validateSubset → resolveRefs → buildApiIR)', () => {
  ...
  it('golden_openapi_invalid_mixed_grouping_expected_failure.yaml → INVALID with OAS_AMBIGUOUS_RESOURCE_GROUPING', () => {
    const yaml = readFileSync(
      join(FIXTURES, 'invalid', 'golden_openapi_invalid_mixed_grouping_expected_failure.yaml'),
      'utf-8'
    );
    const result = checkOpenAPI(yaml);
    expect(result.valid).toBe(false);
  ...
  it('golden_openapi_invalid_non_crud_expected_failure.yaml → INVALID with IR_INVALID (non-CRUD operation)', () => {
    const yaml = readFileSync(
      join(FIXTURES, 'invalid', 'golden_openapi_invalid_non_crud_expected_failure.yaml'),
      'utf-8'
    );
    const result = checkOpenAPI(yaml);
    expect(result.valid).toBe(false);
  ...
  it(
    'valid-specs-api-guru fixtures → all VALID (RUS-v1 compliant from APIs.guru corpus)',
    { timeout: 30000 },
    () => {
