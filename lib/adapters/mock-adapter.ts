...
import type { CrudAdapter, AdapterCapabilities } from "./types";
...
function toAdapterCapabilities(caps?: Capabilities): AdapterCapabilities {
  if (!caps) {
    return { create: true, read: true, update: true, delete: true };
  }
  return {
    create: caps.create,
    read: caps.list || caps.detail,
    update: caps.update,
    delete: caps.delete,
  };
}
...
export function createMockAdapter(
  ...
  return {
    mode: "mock",
    capabilities: toAdapterCapabilities(capabilities),
    async getSample() {
