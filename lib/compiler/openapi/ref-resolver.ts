...
type ResolveResult = { ok: true; value: unknown } | { ok: false; error: CompilerError };

function resolveRef(doc: Record<string, unknown>, ref: string, stack: string[]): ResolveResult {
  if (isExternalRef(ref)) {
    return {
      ok: false,
      error: createError('OAS_EXTERNAL_REF', 'Resolve', `External $ref not supported: ${ref}`, ref),
    };
  }
  if (!ref.startsWith('#/')) {
    return {
  ...
  if (stack.includes(ref)) {
    return {
      ok: false,
      error: createError('OAS_CIRCULAR_REF', 'Resolve', `Circular $ref: ${ref}`, ref),
    };
  }
  ...
  if (target === undefined || target === null) {
    return {
      ok: false,
      error: createError('OAS_EXTERNAL_REF', 'Resolve', `Invalid $ref target: ${ref}`, ref),
    };
  }
...
function transformRefs(obj: unknown, doc: Record<string, unknown>, stack: string[]): ResolveResult {
 ...
 */
export function resolveRefs(doc: Record<string, unknown>): ResolveRefsOutput {
