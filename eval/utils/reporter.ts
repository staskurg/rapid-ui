...
export function generateMarkdownReport(report: EvaluationReport): string {
  ...
  lines.push(`- **Overall Valid Rate:** ${(report.overallValidRate * 100).toFixed(1)}%`);
  lines.push(`- **Overall Stability:** ${(report.overallStability * 100).toFixed(1)}%`);
  lines.push('');
  ...
  report.fixtureResults.forEach((fixture) => {
    ...
    if (!fixture.consistent && fixture.averageSimilarity < 0.9) {
      ...
      lines.push(
        `- Structural drift detected (similarity: ${(fixture.averageSimilarity * 100).toFixed(1)}%)`
      );
...
export function generateTextReport(report: EvaluationReport): string {
  ...
  report.fixtureResults.forEach((fixture) => {
    ...
    const stabilityLabel = fixture.averageSimilarity >= 0.9 ? 'high' : 'low';
    lines.push(
      `  Stability: ${stabilityLabel} (avg: ${(fixture.averageSimilarity * 100).toFixed(1)}%)`
    );
